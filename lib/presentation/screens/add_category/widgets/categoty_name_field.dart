import 'package:finca/application/category/category_form/category_form_bloc.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_hooks/flutter_hooks.dart';
import 'package:line_icons/line_icons.dart';
import 'package:finca/presentation/screens/widgets/custom_textfield.dart';

class CategoryNameField extends HookWidget {
  const CategoryNameField({super.key});

  @override
  Widget build(BuildContext context) {
    final textEditingController = useTextEditingController();
    return BlocListener<CategoryFormBloc, CategoryFormState>(
      listenWhen: (previous, current) =>
          previous.isEditing != current.isEditing,
      listener: (context, state) {
        textEditingController.text =
            state.categoryEntity.categoryName.getOrCrash();
      },
      child: CustomTextField.dark(
        hintText: 'Category name',
        prefixIcon: LineIcons.pollH,
        inputFormatter: <TextInputFormatter>[
          FilteringTextInputFormatter.allow(
            RegExp(r'[a-zA-Z]'),
          ),
        ],
        maxLength: 10,
        controller: textEditingController,
        onChanged: (value) => context.read<CategoryFormBloc>().add(
              CategoryFormEvent.categoryNameChanged(value),
            ),
        validator: (_) => context
            .read<CategoryFormBloc>()
            .state
            .categoryEntity
            .categoryName
            .value
            .fold(
              (f) => f.maybeMap(
                  empty: (f) => 'Cannot be empty', orElse: (() => null)),
              (_) => null,
            ),
      ),
    );
  }
}
